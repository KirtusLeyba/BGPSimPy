...
to national boundaries.  We define \emph{national chokepoint potential} to
...
The paper extends earlier research on AS topology, first by introducing the national chokepoint potential measure, then by presenting open-source cross-platform tools for simulating
...
We are interested in how AS topologies have changed as the total size of the graph has grown from about 10,000 ASes in the early 2000s to over 60,000 today.
The structural
...
\begin{enumerate}
 \item We introduce the measure of chokepoint potential for single border ASes and the aggregate measure national chokepoint potential, 
...
efficiently evaluating chokepoints for a given state of the Internet. 
 \item we show that national chokepoint potential
...
The AS-layer is the highest level of organization of the Internet, with each individual AS represents a separately administered network, for example, a large ISP, a university network, or a government entity. The Border Gateway Protocol (BGP) is used to route traffic among ASes \cite{bgp}.  AS-level routes are known as \emph{paths} and are specified by routing tables stored locally in each BGP AS.  These tables express known connections to other ASes and local preferences for how data are routed, e.g., if there are multiple paths to a single destination, an AS may prefer the path that costs the least.
...
we also need an aggregate measure for comparing nations, which we call \emph{national chokepoint potential}.
...
another measure, with this one being a national measure. We define the \emph{national chokepoint potential} of a nation to be the minimum number
...
that either enter said nation (for the out-to-in case) or exit said nation (for the in-to-out case). National chokepoint potential is calculated taking the strongest ASes first,
...
Our technique provides a comparison more likely to reflect reality.
...
are used to determine the chokepoint potentials for every border AS. Finally, \toolname{} calclates the national chokepoint potential 
for each nation. In our experiments, we used the country codes returned from Team Cymru's
IP to ASN whois service \cite{cymru}. This service only maintains the most recent registration of each AS, so to control for uncertain country to AS links
we keep ASes that have been registered more recently than the current timestam being tested in the path simulation but we do not assign these ASes chokepoint potentials. We chose this route
because the datasets of AS registrations for the various ASN registries tended to have more missing data at specific timestamps.
...
\toolname{} takes the resulting chokepoint potentials and generates national chokepoint potentials for each nation.
...
to 100 (the least free). We compare our results to the FOTN scores by ranking nations according to national chokepoint potential, particulary the number of border ASes required to intercept
...
In our test of the relationship between Internet Freedom and national chokepoint potential, we plotted the Freedom on the Net score
...
\begin{figure}
	...
	\caption{Number of ASes to intercept 90\% of ito paths vs Freedom On The Net Score (2017). 
	Green nations are free, yellow are mostly free, and blue are not free according to FOTN.}\label{fig:fotn2017}
\end{figure}
