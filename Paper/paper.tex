...
sought to control these movements by disconnecting in-country networks from the rest of the world \todo[inline]{need references here}.
...
The AS-layer is the highest level of organization of the Internet, with each individual AS consisting of a separately administered network, which could represent a large ISP, a university network, or a government entity. The Border Gateway Protocol (BGP) is used to route traffic among ASes. \Steph[inline]{need citations}.  AS-level routes are known as \emph{paths} and are specified by routing tables stored locally in each BGP node.  These tables express local preferences for how data are routed, e.g., if there are multiple paths to a single destination, an AS may prefer the path that costs the least.
...
incompleteness and innacuracies \cite{tracerouteProblems}.\Steph[inline]{Kirts: Please explain BGPStream and what the problem with it is.}
\par
